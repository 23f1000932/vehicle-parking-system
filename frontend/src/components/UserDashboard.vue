...
// Reactive States for Lots and Reservations
const lots = ref([]);
...
const renderReservationChart = () => {
  ...
  const completedCount = reservations.value.filter(r => r.status === 'Inactive').length;
...
const fetchLots = async () => {
  try {
    ...
    console.error('Error while fetching lots:', error);
  }
};
...
const makeReservation = async (lotId) => {
  try {
    ...
    console.log('Reservation successful:', response.data.message);
    fetchLots();
    fetchReservations();
    activeTab.value = 'history';
  } catch (error) {
    console.error('Error while making reservation:', error.response?.data?.message || error.message);
  }
};
...
const fetchReservations = async () => {
  ...
  try {
    const response = await axios.get(`${API_URL}/api/users/${authStore.user.id}/reservations`);
...
// Prepare to End Reservation
// This function calculates the cost and prepares the payment modal
const prepareToEndReservation = async (reservation) => {
  try {
    const response = await axios.get(`${API_URL}/api/reservations/${reservation.id}/calculate_cost`);
    paymentDetails.value = {
      reservationId: reservation.id,
      cost: response.data.total_cost,
      lotName: reservation.lot_name,
    };
    showPaymentModal.value = true;
  } catch (error) {
    console.error('Error while calculating cost:', error.response?.data?.message || error.message);
  }
};

const handlePayment = async () => {
  if (!paymentDetails.value.reservationId) return;
  try {
    const response = await axios.put(`${API_URL}/api/reservations/${paymentDetails.value.reservationId}/end`);
    console.log("Payment successful!", response.data.message);
    showPaymentModal.value = false;
    fetchLots();
    fetchReservations();
  } catch (error) {
    console.error('Error in final payment:', error.response?.data?.message || error.message);
  }
};
    ...
    <div class="card">
      <div class="card-header">
        ...
        <ul class="nav nav-tabs card-header-tabs">
          <li class="nav-item">
            <a class="nav-link" :class="{ active: activeTab === 'find' }" @click="activeTab = 'find'" href="#">Find a Spot</a>
          </li>
          <li class="nav-item">
            <a class="nav-link" :class="{ active: activeTab === 'history' }" @click="activeTab = 'history'" href="#">My Reservations</a>
          </li>
          <li class="nav-item">
          ...
          <div v-for="lot in lots" :key="lot.id" class="col-md-6 col-lg-4 mb-4">
            <div class="card h-100">
              ...
              <div class="card-footer">
                <button @click="makeReservation(lot.id)" class="btn btn-primary w-100" :disabled="lot.available_spots === 0">
      ...
      <div v-if="activeTab === 'history'" class="card-body">
          ...
          <table class="table table-striped">
            ...
            <tbody>
              <tr v-for="res in reservations" :key="res.id">
                <td>{{ res.lot_name }}</td>
                <td>{{ res.lot_address }}</td>
                <td>{{ formatDate(res.time_in) }}</td>
                <td>{{ formatDate(res.time_out) }}</td>
                <td>
                ...
                <td>
                  <button v-if="res.status === 'Active'" @click="prepareToEndReservation(res)" class="btn btn-info btn-sm">
                    End Reservation
                  </button>
                </td>
              </tr>
        ...
        <p v-if="reservations.length === 0">You have no reservation history.</p>
      </div>
    ...
    <div class="modal-dialog modal-dialog-centered">
      <div class="modal-content">
        ...
        <div class="modal-body">
          <p>You are about to end your reservation for <strong>{{ paymentDetails.lotName }}</strong>.</p>
          <h4>Total Cost: ₹{{ paymentDetails.cost.toFixed(2) }}</h4>
          <p class="text-muted small">This is a simulated payment.</p>
        </div>
        <div class="modal-footer">
          <button type="button" class="btn btn-secondary" @click="showPaymentModal = false">Cancel</button>
          <button type="button" class="btn btn-success" @click="handlePayment">Pay Now</button>
        </div>
      </div>
    </div>
  </div>
